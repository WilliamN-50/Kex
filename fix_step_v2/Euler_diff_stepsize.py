...
def main():
    ...
    # model.load_state_dict(torch.load("../trained_model/eq_1_model_50_Adam_500_noise_01.pth"))
    model.load_state_dict(torch.load("eq_1_model_Adam_no_noise_15_3.pth"))
    model.eval()

    t_dem_adap, y_dem_adap, i = adaptive_euler(model=model, t0=t0, t_end=t_end, y0=y0, h=0.1, tol=0.0001, diff_eq=diff_eq,
                                            device=device)

    t_dem_fix, y_dem_fix = fix_euler_method(model=model, t0=t0, t_end=t_end, y0=y0, h=(t_end-t0)/(len(t_dem_adap)), diff_eq=diff_eq,
                                            device=device)
    # t_euler_fix, y_euler_fix = fix_euler_method(model=model, t0=t0, t_end=t_end, y0=y0, h=(t_end-t0)/(len(t_dem_adap)), diff_eq=diff_eq,
